...
ggplot() +
  ...
  geom_line(agg_energy_demand_tax, mapping = aes(x=Qagg_tax, y = price_tax, color = "taxed agg. demand")) + #taxed agg demand
  geom_abline(slope = coef(low_income)[["q_low_kwh"]], 
              intercept = 1.97 + coef(low_income)[["(Intercept)"]], color="purple", linetype="twodash") + # Low consumer demand
  geom_abline(slope = coef(high_income)[["q_high_kwh"]], 
              intercept = 1.97 + coef(high_income)[["(Intercept)"]], color="steelblue", linetype="twodash") + # High consumer demand
  geom_abline(slope = (10/market_q), intercept = 0, color = "orange", aes(value = "supply")) + # supply curve
  geom_abline(slope = 0, intercept = MEC, color = "darkolivegreen3", aes(value = "MEC")) + # MEC
  scale_x_continuous(limits = c(0,850000), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0,35), expand = c(0, 0)) +
...
```{r}
tax_pstar <-tax_intersect[2]
tax_qstar <- tax_intersect[1]

high_sum <- summary.lm(high_income)
high_yint <- (high_sum$coefficients[1,1]) - MEC
CS_high_tax <- 0.5*tax_qstar*((high_yint-tax_pstar + MEC)/100)

low_sum <- summary.lm(low_income)
low_yint <- (low_sum$coefficients[1,1]) - MEC
CS_low_tax <- 0.5*tax_qstar*((low_yint-tax_pstar + MEC)/100)
CS_low_tax_ec <- CS_low_tax - (env_cost/100)
```
(c) Overall welfare of "high" income consumers: **$ `r round(CS_high_tax, 2)` **
(d) Overall welfare of "low" income consumers **$ `r round(CS_low_tax_ec, 2)` **
